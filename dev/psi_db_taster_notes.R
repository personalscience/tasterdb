...
source(file.path(here::here(),"dev", "psi_taster_read_data.R"))  # load functions to process Tastermonial files
...
taster_notes2_df <- taster_raw2_df %>% filter(!is.na(user))  %>%  transmute(Start = with_tz(lubridate::parse_date_time(startEatingDate, orders = c("mdy HM", "Ymd HM")),
                                                            tzone = Sys.timezone()),
                                            End = as_datetime(NA),
                                            Activity = "Food",
...
taster_notes_df1 <- taster_raw_df %>% transmute(Start = with_tz(lubridate::parse_date_time(startEatingDate, orders = c("dmY HM p z", "dmY HM z")),
                                                         tzone = Sys.timezone()),
                                         End = as_datetime(NA),
                                         Activity = "Food",
                                         Comment = str_to_upper(str_squish(str_replace_all(name, "[^A-Za-z,]", " "))),
